#!/usr/bin/env bash
#
# Add a court/custody case to 2020s-era prisoner records that currently have
# NONE. A full-database audit found 472 records with no case attached; this is
# the first batch — 110 cases for the 2020s cohort (George Floyd uprising, J6,
# Boogaloo, ICE-detention and related defendants), structured from each
# record's own description and verified/refined against DOJ releases and court
# records. The data lives in database/data/cases-2020s-batch1.json.
#
...
$rows = json_decode(file_get_contents(base_path("database/data/cases-2020s-batch1.json")), true);
...
echo
echo "Done. 2020s caseless records enriched (batch 1)."
