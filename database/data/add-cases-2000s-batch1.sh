#!/usr/bin/env bash
#
# Add a court/custody case to 2000s-era prisoner records that currently have
# NONE. A full-database audit found 472 records with no case attached; this
# covers the 2000s cohort — 136 cases, dominated by Vieques Navy-protest and
# SOA Watch trespass defendants (18 U.S.C. 1382, D.P.R./M.D. Ga.), plus the
# RNC 8, nuclear/peace resisters, and others — structured from each record's
# own description and refined against court records where possible. Many
# Vieques/SOA entries carry the charge, court and facility but no individual
# dates (only campaign-wide ranges existed). The data lives in
# database/data/cases-2000s-batch1.json.
#
...
$rows = json_decode(file_get_contents(base_path("database/data/cases-2000s-batch1.json")), true);
...
echo
echo "Done. 2000s caseless records enriched (batch 1)."
