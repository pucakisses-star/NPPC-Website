#!/usr/bin/env bash
#
# Add a court/custody case to the remaining caseless prisoner records, across
# the smaller eras (1700s-2010s: colonial rebellions, abolitionists, IWW/WWI,
# Sacco & Vanzetti, Smith Act, civil-rights and later). This is the final
# batch of the caseless sweep — 72 cases, structured from each record's own
# description and refined against court records where possible; acquittals,
# pardons and executions are recorded with the outcome in the case. The data
# lives in database/data/cases-historical-batch1.json.
#
...
$rows = json_decode(file_get_contents(base_path("database/data/cases-historical-batch1.json")), true);
...
echo
echo "Done. caseless records enriched (final historical batch)."
