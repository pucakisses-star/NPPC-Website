...
<style>
    ...
    .cal-header { display: flex; align-items: center; justify-content: space-between; padding: 48px 0 40px; border-bottom: 1px solid rgba(var(--fg-rgb),0.1); margin-bottom: 48px; }
    .cal-header-left { display: flex; align-items: center; gap: 16px; }
    .cal-header-title { font-size: 18px; font-weight: 800; color: var(--fg); }
    .cal-toggle { display: flex; align-items: center; gap: 12px; }
    .cal-toggle-label { font-size: 14px; font-weight: 600; color: rgba(var(--fg-rgb),0.5); }
    .cal-toggle-label.active { color: var(--fg); }
    .cal-toggle-switch { width: 48px; height: 26px; background: var(--surface-2); border-radius: 13px; cursor: pointer; position: relative; transition: background 0.2s; }
    .cal-toggle-switch.on { background: var(--accent); }
    ...
    .cal-month-name { font-size: 2.5rem; font-weight: 900; color: var(--fg); }
    .cal-month-btn { background: rgba(var(--fg-rgb),0.06); border: 1px solid rgba(var(--fg-rgb),0.15); color: var(--fg); width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; cursor: pointer; border-radius: 4px; text-decoration: none; font-size: 18px; transition: background 0.15s; }
    .cal-month-btn:hover { background: rgba(var(--fg-rgb),0.12); }
    .cal-months-bar { display: flex; gap: 4px; flex-wrap: wrap; margin-bottom: 32px; }
    .cal-months-btn { padding: 6px 16px; font-size: 13px; font-weight: 600; border: 1px solid rgba(var(--fg-rgb),0.15); color: rgba(var(--fg-rgb),0.6); background: transparent; cursor: pointer; border-radius: 4px; text-decoration: none; transition: all 0.15s; }
    .cal-months-btn:hover { border-color: var(--accent); color: var(--fg); }
    .cal-months-btn.active { background: var(--accent); border-color: var(--accent); color: var(--on-accent); }

    /* Month grid */
    .cal-grid { display: grid; grid-template-columns: repeat(4, 1fr); grid-auto-rows: 1fr; gap: 24px; padding-bottom: 80px; }
    .cal-card { border: 1px solid rgba(var(--fg-rgb),0.12); border-radius: 4px; overflow: hidden; transition: border-color 0.2s; cursor: pointer; text-decoration: none; display: flex; flex-direction: column; height: 100%; }
    .cal-card:hover { border-color: rgba(var(--fg-rgb),0.3); }
    .cal-card.today { border-color: var(--accent); border-width: 2px; }
    ...
    .cal-card-day { font-size: 4rem; font-weight: 900; color: var(--fg); line-height: 1; margin-bottom: 12px; }
    .cal-card.today .cal-card-day { color: var(--accent); }
    .cal-card-title { font-size: 14px; font-weight: 600; color: rgba(var(--fg-rgb),0.85); line-height: 1.4; }
    .cal-card-divider { width: 24px; height: 2px; background: rgba(var(--fg-rgb),0.3); margin: 12px auto 6px; }
    .cal-card-year { font-size: 13px; color: rgba(var(--fg-rgb),0.5); font-weight: 600; }
    ...
    .cal-empty-card { border: 1px dashed rgba(var(--fg-rgb),0.08); border-radius: 4px; display: flex; align-items: center; justify-content: center; min-height: 280px; height: 100%; }
    .cal-empty-day { font-size: 4rem; font-weight: 900; color: rgba(var(--fg-rgb),0.06); line-height: 1; }

    /* Day view */
    .cal-day-view { display: flex; gap: 0; min-height: 70vh; padding-bottom: 80px; }
    .cal-day-left { flex: 0 0 320px; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; border-right: 1px solid rgba(var(--fg-rgb),0.1); padding: 48px; position: sticky; top: 108px; align-self: flex-start; max-height: calc(100vh - 108px); overflow-y: auto; }
    .cal-day-num { font-size: 8rem; font-weight: 900; color: var(--fg); line-height: 1; }
    .cal-day-month { font-size: 4rem; font-weight: 900; color: var(--fg); line-height: 1; text-transform: uppercase; }
    .cal-day-share { margin-top: 48px; text-align: center; }
    .cal-day-share-title { font-size: 16px; font-weight: 800; color: var(--fg); margin-bottom: 8px; }
    .cal-day-share-text { font-size: 13px; color: rgba(var(--fg-rgb),0.5); margin-bottom: 16px; line-height: 1.5; }
    .cal-day-right { flex: 1; padding: 48px 64px; }
    .cal-day-dateline { font-size: 14px; color: rgba(var(--fg-rgb),0.5); margin-bottom: 24px; display: flex; align-items: center; gap: 12px; }
    .cal-day-dateline-bar { width: 24px; height: 2px; background: rgba(var(--fg-rgb),0.3); }
    .cal-day-title { font-size: 2.5rem; font-weight: 900; color: var(--fg); line-height: 1.15; margin-bottom: 32px; text-align: center; }
    ...
    .cal-day-desc { font-size: 16px; color: rgba(var(--fg-rgb),0.7); line-height: 1.8; max-width: 600px; margin: 0 auto; }
    .cal-day-nav { display: flex; gap: 12px; justify-content: center; margin-top: 40px; }
    .cal-day-nav-btn { padding: 10px 24px; border: 1px solid rgba(var(--fg-rgb),0.2); color: var(--fg); text-decoration: none; font-size: 14px; font-weight: 600; border-radius: 4px; transition: all 0.15s; }
...
<div class="cal-page">
        ...
        <div class="cal-day-view">
                ...
                <div class="cal-day-share">
                    ...
                    <div style="display:flex; justify-content:center; gap:14px; margin-top:14px; flex-wrap:wrap;">
                        ...
                        <a href="{{ $twitterUrl }}" target="_blank" rel="noopener" title="Share on X (Twitter)" aria-label="Share on X"
                           style="width:36px; height:36px; display:inline-flex; align-items:center; justify-content:center; border:1px solid rgba(var(--fg-rgb),0.15); border-radius:50%; color:rgba(var(--fg-rgb),0.75); transition:all 0.15s;"
                           onmouseover="this.style.color='var(--fg)'; this.style.borderColor='rgba(var(--fg-rgb),0.4)';"
                           onmouseout="this.style.color='rgba(var(--fg-rgb),0.75)'; this.style.borderColor='rgba(var(--fg-rgb),0.15)';">
            ...
            <div class="cal-day-right">
                ...
                @foreach($dayEntries as $idx => $entry)
                    @if($idx > 0)
                        <hr style="border:none; border-top:1px solid rgba(var(--fg-rgb),0.1); margin:48px 0;">
                    @endif
                    @if($dayEntries->count() > 1)
                        <div style="font-size:13px; font-weight:700; text-transform:uppercase; letter-spacing:0.08em; color:rgba(var(--fg-rgb),0.4); margin-bottom:12px;">{{ $monthName }} {{ str_pad($selectedDay, 2, '0', STR_PAD_LEFT) }}, {{ $entry->year }}</div>
                    @endif
                    ...
                    @if($entry->prisoner)
                        ...
                        <div style="border:1px solid rgba(var(--fg-rgb),0.1); border-radius:8px; padding:24px; margin:32px auto; max-width:600px; display:flex; gap:20px; align-items:flex-start;">
                            @if($p->photo)
                                <img src="{{ asset('storage/'.$p->photo) }}" style="width:80px; height:80px; border-radius:50%; object-fit:cover; flex-shrink:0;">
                            @endif
                            <div style="flex:1;">
                                <a href="/prisoner/{{ $p->slug ?: $p->id }}" style="font-size:18px; font-weight:800; color:var(--fg); text-decoration:none; display:block; margin-bottom:4px;">{{ $p->name }}</a>
                                @if($p->aka)<div style="font-size:13px; color:rgba(var(--fg-rgb),0.4); margin-bottom:8px; font-style:italic;">AKA: {{ $p->aka }}</div>@endif
                                ...
                                @if($p->description)
                                    <div style="font-size:14px; color:rgba(var(--fg-rgb),0.6); line-height:1.6;">{{ \Illuminate\Support\Str::limit(strip_tags($p->description), 200) }}</div>
                                @endif
                                <a href="/prisoner/{{ $p->slug ?: $p->id }}" style="display:inline-block; margin-top:12px; font-size:13px; font-weight:700; color:var(--accent-2); text-decoration:none;">View full profile &rarr;</a>
                            </div>
                        </div>
                    @endif
    ...
    @elseif($view === 'day' && $dayEntries->isEmpty())
        <div style="text-align:center; padding:80px 0; color:rgba(var(--fg-rgb),0.4);">No entry for this day. Try the month view to see all entries.</div>
    @else
        {{-- MONTH VIEW --}}
        <div class="cal-grid">
            @php
            ...
            @for($d = 1; $d <= $daysInMonth; $d++)
                @if(isset($entriesByDay[$d]))
                    ...
                    <a href="/calendar?month={{ $month }}&view=day&day={{ $d }}" class="cal-card {{ ($month === $currentMonth && $d === $today) ? 'today' : '' }}" data-no-fade style="position:relative;">
                        @if($extra > 0)
                            <span style="position:absolute; top:8px; right:8px; font-size:11px; font-weight:700; padding:2px 6px; border-radius:10px; background:rgba(86,96,254,0.2); color:var(--accent-2); border:1px solid rgba(86,96,254,0.4);">+{{ $extra }}</span>
                        @endif
                        <div class="cal-card-top">
                            @if($entry->prisoner && $entry->prisoner->photo)
                                <img src="{{ asset('storage/'.$entry->prisoner->photo) }}" style="width:40px; height:40px; border-radius:50%; object-fit:cover; margin:0 auto 8px; display:block; border:2px solid rgba(var(--fg-rgb),0.15);">
                            @endif
