...
<style>
    ...
    .nls-frame-line { position: absolute; background: var(--accent); display: block; transform-origin: top left; transition: transform 0.9s cubic-bezier(.59,.08,.39,.95); }
    ...
    .nls-brand { font-size: 12px; font-weight: 800; color: var(--accent); text-transform: uppercase; letter-spacing: 0.16em; margin-bottom: 14px; }
    .nls-compact .nls-brand { font-size: 11px; margin-bottom: 10px; }
    .nls-title { font-size: 2rem; font-weight: 900; color: var(--fg); line-height: 1.1; margin: 0 0 14px; }
    .nls-compact .nls-title { font-size: 1.35rem; margin-bottom: 16px; }
    .nls-lede { font-size: 15px; color: rgba(var(--fg-rgb),0.65); line-height: 1.6; margin: 0 0 24px; max-width: 560px; }

    .nls-form { display: flex; gap: 0; max-width: 560px; }
    .nls-input { flex: 1; background: rgba(var(--fg-rgb),0.04); border: 1px solid rgba(var(--fg-rgb),0.18); border-right: 0; border-radius: 4px 0 0 4px; padding: 14px 16px; color: var(--fg); font-size: 15px; outline: none; }
    .nls-input:focus { border-color: var(--accent); }
    .nls-input::placeholder { color: rgba(var(--fg-rgb),0.4); }
    .nls-btn { background: #ff5851; color: var(--on-accent); border: 1px solid #ff5851; border-radius: 0 4px 4px 0; padding: 14px 28px; font-size: 14px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.08em; cursor: pointer; transition: background 0.15s, border-color 0.15s; }
    ...
    @media (max-width: 640px) {
        ...
        .nls-input { border-right: 1px solid rgba(var(--fg-rgb),0.18); border-radius: 4px; }
