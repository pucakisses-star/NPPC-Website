...
@if($featuredEvent)
    ...
    <style>
        ...
        .home-event-eyebrow { font-size: 1.05rem; font-weight: 800; letter-spacing: 0.06em; text-transform: uppercase; color: var(--fg); }
        .home-event-more { font-size: 13px; font-weight: 700; letter-spacing: 0.04em; text-transform: uppercase; color: var(--accent-2); text-decoration: none; }
        .home-event-more:hover { color: var(--fg); }
        .home-event-rule { position: relative; height: 1px; background: rgba(var(--fg-rgb),0.18); margin: 10px 0 36px; }
        .home-event-rule::before { content: ""; position: absolute; left: 0; top: -1px; height: 3px; width: 110px; background: var(--accent); }
        ...
        .home-event-visual-fallback { width: 100%; height: 100%; display: flex; flex-direction: column; justify-content: center; padding: 40px; color: var(--on-dark); }
        ...
        .home-event-tag { display: inline-block; font-size: 11px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: var(--accent-2); background: rgba(86,96,254,0.18); border: 1px solid rgba(86,96,254,0.4); padding: 4px 10px; border-radius: 3px; margin-bottom: 14px; }
        .home-event-date { font-size: 2.4rem; font-weight: 800; color: var(--accent-2); line-height: 1; }
        .home-event-title { font-size: 2.1rem; font-weight: 800; color: var(--fg); line-height: 1.12; margin: 4px 0 16px; }
        .home-event-desc { font-size: 16px; line-height: 1.6; color: rgba(var(--fg-rgb),0.75); margin: 0 0 16px; }
        .home-event-meta { display: flex; flex-direction: column; gap: 2px; font-size: 14px; color: rgba(var(--fg-rgb),0.6); margin-bottom: 20px; }
        .home-event-cta { display: inline-block; font-size: 14px; font-weight: 700; letter-spacing: 0.04em; text-transform: uppercase; color: var(--accent-2); text-decoration: none; }
        .home-event-cta:hover { color: var(--fg); }
